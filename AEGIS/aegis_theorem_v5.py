...
"""
AEGIS‑Theorem v5 – integer identity search with correct simplification.
No trig, no constants‑only expressions.
"""
...
class Node:
    ...
    def simplify(self):
        """Simplify the expression tree – handles constants and identity rules."""
        if self.is_terminal():
            return self
        ...
        # If both children are numeric constants, evaluate.
        ...
        # Identity simplifications (only when the child is numeric constant)
        if self.value == '+':
...
class GeneticProgram:
    ...
    def fitness(self, node, data):
        ...
        # Reject if it's a constant (no variables)
        def has_var(n):
            if n.is_terminal():
        ...
        # Reject if it's identically zero on a few test points
...
def main():
    print("="*72)
    print("AEGIS‑Theorem v5 – integer identity search")
    print("="*72)

...
"""
AEGIS Discovery Engine v5 – GP with Parsimony Pressure

This is a complete symbolic regression engine that:
- Searches over expressions with +, -, *, /, sin, cos, exp, log, x, constants
- Uses parsimony pressure to prefer simpler expressions
- Validates with three gates (rate, reconstruction, prediction)
- Detects singularities and invalid operations
- Promotes only if all gates pass AND expression is valid

The "Rain Man" property: it finds the simplest expression that fits.
"""
...
class Node:
    ...
    def has_division_by_zero(self, x):
        """Check if this expression is undefined at x."""
        try:
            evaluate_node(self, x)
            return False
    ...
    def simplify(self):
        """Simplify the expression tree (remove redundant ops)."""
        if self.is_terminal():
            return self
...
def evaluate_node(node, x):
    """Evaluate expression tree at x with proper error handling."""
    if node.is_terminal():
        if node.value == 'x':
            return x
        else:
    ...
    if node.value == 'exp':
        ...
        if val > 709:
            raise OverflowError(f"exp({val}) overflow at x={x}")
        return math.exp(val)

    if node.value == 'log':
        val = evaluate_node(node.left, x)
        if val <= 0:
            raise ValueError(f"log({val}) undefined at x={x}")
        return math.log(val)
...
def optimize_constants(expr_node, x_data, y_data, max_iter=50):
    """Refine constants using coordinate descent."""
    constants = expr_node.collect_constants()
    if not constants:
        return expr_node

    best_node = expr_node.clone()
    best_fitness = fitness_with_penalty(best_node, x_data, y_data)
        ...
        for idx in const_order:
            ...
            for delta in [-0.1, 0.1, -0.01, 0.01, -0.001, 0.001]:
                ...
                try:
                    ...
                    continue
                fit = fitness_with_penalty(expr_node, x_data, y_data)
                if fit < best_fitness:
                    best_fitness = fit
                    best_node = expr_node.clone()
                    improved = True
...
def fitness_with_penalty(node, x_data, y_data, lambda_complexity=0.01):
    """
    Fitness = RMSE + lambda * node_count
    This pushes evolution toward simpler expressions.
    """
    try:
        total = 0.0
...
class GeneticProgram:
    def __init__(self, population_size=300, generations=150, max_depth=6,
    ...
    def random_tree(self, depth):
        if depth >= self.max_depth or (depth > 0 and random.random() < 0.25):
            # Terminal
    ...
    def fitness(self, node, x_data, y_data):
        return fitness_with_penalty(node, x_data, y_data, self.lambda_complexity)
...
class AEGIS_Discovery_v5:
    ...
    def validate_candidate(self, expr_node, x_data, y_data) -> Optional[Dict]:
        ...
        # Check for undefined points (singularities)
...
def main():
    ...
    print("=" * 72)
    print("AEGIS DISCOVERY ENGINE v5 – GP with Parsimony Pressure")
    print("=" * 72)
    print()

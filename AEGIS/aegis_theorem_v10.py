...
"""
AEGIS‑Theorem v10 – no division, larger training set, exact zero.
Operators: +, -, *
Constants: integers -10..10
Training bound: 5
"""
...
class GeneticProgram:
    ...
    def fitness(self, node, data):
        ...
        # Must contain at least two different variables
        def vars_used(n):
            if n.is_terminal():
        ...
        if len(vars_used(simplified)) < 2:
            return float('inf')
        # Exact zero on all training points
        for x, y, z in data:
            if simplified.evaluate_int(x, y, z) != 0:
                return float('inf')
        # Perfect – fitness = node count (simpler is better)
    ...
    def evolve(self, data):
        self.population = [self.random_tree(0) for _ in range(self.pop_size)]
        self.best_individual = None
        self.best_fitness = float('inf')
...
def main():
    print("="*72)
    print("AEGIS‑Theorem v10 – no division, bound=5")
    print("="*72)
    bound = 5

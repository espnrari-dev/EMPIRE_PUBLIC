...
"""
AEGIS Discovery Engine v7 – Prudent Discovery
Warren Buffett / Charlie Munger style:
- Simplicity: penalize complexity strongly.
- Margin of safety: must beat constant baseline by >50%.
- Uncertainty: bootstrap confidence intervals.
- Stability: cross-validation structural check.
- Prudence score: composite 0-100; promote only if >80.
"""
...
# ============================================================
# NODE REPRESENTATION (same as V6)
# ============================================================
...
# ============================================================
# SYMBOLIC DERIVATIVE (same as V6)
# ============================================================
...
# ============================================================
# CONSTANT OPTIMIZATION (same as V6)
# ============================================================
...
# ============================================================
# GENETIC PROGRAMMING (adapted for prudence)
# ============================================================
...
def margin_of_safety(expr_rmse, baseline_rmse):
    """Margin of safety: how much better than baseline."""
    if baseline_rmse == 0:
        return float('inf')
    return (baseline_rmse - expr_rmse) / baseline_rmse

def bootstrap_ci(model_func, x_data, y_data, n_bootstrap=100):
    """Bootstrap confidence interval for predicted y at each x."""
    n = len(x_data)
    preds = []
    for _ in range(n_bootstrap):
        idx = [random.randint(0, n-1) for _ in range(n)]
        x_boot = [x_data[i] for i in idx]
        y_boot = [y_data[i] for i in idx]
        # Refit constants on bootstrap sample
        model_clone = model_func.clone()
        opt = optimize_constants(model_clone, x_boot, y_boot, max_iter=50)
        # Predict on original x
        pred = [evaluate_node(opt, x) for x in x_data]
        preds.append(pred)
    # Compute std at each x
    stds = []
    for i in range(len(x_data)):
        vals = [p[i] for p in preds]
        stds.append(math.sqrt(sum((v - sum(vals)/len(vals))**2 for v in vals) / len(vals)))
    return stds

def cross_validate_structure(x_data, y_data, model_node, n_folds=3):
    """Check if the model structure (simplified) remains similar across folds."""
    ...
    for i in range(n_folds):
        ...
        # Discover on train (quickly)
        gp = GeneticProgram(population_size=200, generations=50, max_depth=5)
        best = gp.evolve(list(x_train), list(y_train))
        structures.append(best.simplify().node_count())
    # If the node count varies more than 20%, reject
    if max(structures) - min(structures) > 0.2 * sum(structures)/len(structures):
        return False
    return True

# ============================================================
# MAIN DISCOVERY ENGINE WITH PRUDENCE
# ============================================================

class AEGIS_Discovery_v7:
    def __init__(self):
    ...
    def validate_candidate(self, expr_node, x_data, y_data):
        # Same three gates as before, but also compute baseline and margin
        ...
        model_rmse = self.rmse(actual_y, recon_y)  # using reconstruction error as overall fit
        margin = margin_of_safety(model_rmse, baseline)
        # Margin of safety: must be > 0.5 (i.e., model explains >50% of baseline error)
        mos_ok = margin > 0.5 if baseline > 0 else False
    ...
    def run(self, x_values: List[float], y_values: List[float]) -> Dict:
        ...
        if not val_res['mos_ok']:
            return {
                "status": "REJECTED",
                "reason": f"Margin of safety too low ({(val_res['margin_of_safety']*100):.1f}% improvement over baseline, need >50%)."
            }
        ...
        # Bootstrap uncertainty (optional, but we can compute a prudence score)
        # For simplicity, we check node count: if too large, reject
        if best_expr.node_count() > 15:
            return {"status": "REJECTED", "reason": f"Expression too complex (nodes: {best_expr.node_count()})."}

        # Prudence score: based on gates, margin, complexity
        gates = val_res['gates_passed'] / 3.0
        margin = min(1.0, val_res['margin_of_safety'])
        complexity_penalty = 1.0 - (best_expr.node_count() - 1) / 20.0  # max 20 nodes
        if complexity_penalty < 0:
            complexity_penalty = 0
        prudence_score = (gates * 40 + margin * 40 + complexity_penalty * 20)
        prudence_score = min(100, prudence_score)

        if prudence_score < 80:
            return {
                "status": "REJECTED",
                "reason": f"Prudence score {prudence_score:.1f} < 80. Need more certainty.",
                "prudence_score": prudence_score
            }

        # If we reach here, promote
        return {
            "status": "PROMOTED",
...
def main():
    random.seed(42)
    print("="*72)
    print("AEGIS DISCOVERY ENGINE v7 – Prudent Discovery")
    print("="*72)
    print()

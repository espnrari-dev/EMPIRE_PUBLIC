...
"""
AEGIS‑Theorem v13 – enhanced with operators, novelty filter, multi‑bound.
"""
...
class Node:
    ...
    def __repr__(self):
        if self.is_terminal():
            ...
            return str(self.value)
        # Special representation for unary operators
...
class GeneticProgram:
    ...
    def seed_valid_identity(self):
        # (x - x) * y  is a valid zero identity
        x = Node('x')
        y = Node('y')
        zero = Node('-', x, x.clone())
        return Node('*', zero, y)
    ...
    def fitness(self, node, data_list):
        ...
        # 2. Must not simplify to a constant (like 0, 1, etc.)
        ...
        # 4. Novelty filter: reject expressions that are equivalent to simple trivial identities
        # Check if expression is of the form A - A or A * 0 etc.
        # We'll use a simple string representation check: if repr(simplified) contains certain patterns
        rep = repr(simplified)
        ...
        if rep.startswith('(') and ' - ' in rep:
            ...
            if len(parts) == 2 and parts[0] == parts[1]:
                return float('inf')
        # Detect multiplication by zero: 0 * anything or anything * 0
        if '* 0' in rep or '0 *' in rep:
            return float('inf')

        # 5. Fitness = node_count - (variables_count * 5) to reward variable diversity
        return simplified.node_count() - (len(variables) * 5)
...
def main():
    print("="*72)
    print("AEGIS‑Theorem v13 – enhanced operators, novelty filter, multi‑bound")
    print("="*72)
    bounds = [3, 4, 5]

...
"""
AEGIS‑Theorem v7 – integer arithmetic only, exact zero requirement.

We use integer arithmetic (no floats) and require the expression to evaluate to
exactly zero on all training triples. No floating-point approximations allowed.
"""
...
class Node:
            ...
            if left.is_terminal() and right.is_terminal():
                if left.value not in ['x','y','z'] and right.value not in ['x','y','z']:
                    ...
                    if self.value == '+':
                        ...
                        if b != 0:
                            return Node(a // b) if a % b == 0 else Node(0)  # exact division only
    ...
    def evaluate_int(self, x, y, z):
        ...
        if self.value == '+':
            ...
            if b == 0:
                return 0  # division by zero -> invalid
            if a % b != 0:
                return 0  # not an integer identity
            return a // b
        return 0
...
class GeneticProgram:
    def __init__(self, population_size=400, generations=200, max_depth=6):
        ...
        self.constants = list(range(-20, 21))  # integers only
        self.population = []
    ...
    def random_tree(self, depth):
        if depth >= self.max_depth or (depth > 0 and random.random() < 0.3):
            # Terminal: choose from terminals or integer constants
    ...
    def fitness(self, node, data):
        ...
        # Evaluate on training data – require EXACT zero for all points
        total_error = 0
        for x, y, z in data:
        ...
        if total_error == 0:
            # This is a perfect match on training data – we want to prefer simpler ones
            return simplified.node_count() * 0.001
        else:
            return float(total_error) + 1000  # huge penalty for non-zero
...
def main():
    print("="*72)
    print("AEGIS‑Theorem v7 – integer arithmetic, exact zero")
    print("="*72)

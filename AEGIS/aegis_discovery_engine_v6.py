...
"""
AEGIS Discovery Engine v6 – Rain Man Edition
- Larger population (500), more generations (200)
- Adaptive parsimony pressure (starts low, increases)
- Post-evolution simplification and re-optimization
- Transcedental operators included
...
def evaluate_node(node, x):
    ...
    if node.value == '/':
        ...
        if abs(denom) < 1e-12:
            raise ZeroDivisionError(f"Division by zero at x={x}")
...
def optimize_constants(expr_node, x_data, y_data, max_iter=100):
    ...
    best_fitness = fitness_with_penalty(best_node, x_data, y_data, 0.0)
        ...
        for idx in const_order:
            ...
            for delta in [-0.1, 0.1, -0.01, 0.01, -0.001, 0.001]:
                ...
                try:
                    ...
                    continue
                fit = fitness_with_penalty(expr_node, x_data, y_data, 0.0)
                if fit < best_fitness:
                    best_fitness = fit
                    best_node = expr_node.clone()
                    improved = True
...
def fitness_with_penalty(node, x_data, y_data, lambda_complexity=0.0):
    try:
        total = 0.0
        for x, y in zip(x_data, y_data):
            pred = evaluate_node(node, x)
            total += (pred - y) ** 2
        rmse = math.sqrt(total / len(x_data))
        complexity = node.node_count()
        return rmse + lambda_complexity * complexity
    except:
        return float('inf')
...
class GeneticProgram:
    def __init__(self, population_size=500, generations=200, max_depth=6):
    ...
    def random_tree(self, depth):
        if depth >= self.max_depth or (depth > 0 and random.random() < 0.2):
            if random.random() < 0.5:
                return Node('x')
            else:
                return Node(random.uniform(-5.0, 5.0))
        else:
    ...
    def fitness(self, node, x_data, y_data, lambda_complexity):
        return fitness_with_penalty(node, x_data, y_data, lambda_complexity)

    def tournament_select(self, fitnesses, tournament_size=7):
    ...
    def evolve(self, x_data, y_data):
        ...
        for gen in range(self.generations):
            # Annealing parsimony pressure: starts low, gradually increases
            lambda_complexity = 0.001 * (gen / self.generations)
            fitnesses = [self.fitness(ind, x_data, y_data, lambda_complexity) for ind in self.population]
            ...
            if fitnesses[min_idx] < self.best_fitness:
                ...
                opt_fitness = self.fitness(optimized, x_data, y_data, lambda_complexity)
            ...
            if gen % 20 == 0:
                print(f"  Gen {gen}: best fitness = {self.best_fitness:.4f}, "
                      f"nodes = {self.best_individual.node_count() if self.best_individual else 0}")
        ...
        # Final optimization with no parsimony pressure
        ...
        self.best_fitness = self.fitness(self.best_individual, x_data, y_data, 0.0)
...
class AEGIS_Discovery_v6:
    ...
    def validate_candidate(self, expr_node, x_data, y_data):
        ...
        # Check for singularities at test points
        test_points = [o.x for o in val_obs] + [0.0, 1.0, -1.0]
    ...
    def run(self, x_values: List[float], y_values: List[float]) -> Dict:
        ...
        print("[GP] Starting Rain Man evolution...")
        gp = GeneticProgram(population_size=500, generations=200, max_depth=6)
        best_expr = gp.evolve(x_train, y_train)

        print(f"[GP] Final expression: {repr(best_expr)}")
        print(f"[GP] Node count: {best_expr.node_count()}")
        print(f"[GP] Depth: {best_expr.depth()}")
        ...
        if validation_result is None:
            return {
                "status": "REJECTED",
                "reason": "Expression has singularities or validation failed."
            }

        if validation_result['gates_passed'] == 3:
            return {
                "status": "PROMOTED",
                "expression": repr(validation_result['expr']),
                "derivative": repr(validation_result['deriv']),
...
def main():
    ...
    print("=" * 72)
    print("AEGIS DISCOVERY ENGINE v6 – Rain Man Edition")
    print("=" * 72)
    print()

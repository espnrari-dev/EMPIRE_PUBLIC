...
"""
AEGIS‑Theorem v19 – reward any zero identity, penalize trivial ones.
"""
...
class GeneticProgram:
    def __init__(self, pop_size=600, generations=400, max_depth=8):
    ...
    def fitness(self, node, data_list):
        # Check modulo‑by‑zero on original tree
        if self.contains_mod_zero(node):
            return float('-inf')

        # 1. Must contain at least two distinct variables (original tree)
        def vars_used(n):
            if n.is_terminal():
                return {n.value} if n.value in ['x','y','z'] else set()
            return vars_used(n.left) | vars_used(n.right) if n.right else vars_used(n.left)
        variables = vars_used(node)
        if len(variables) < 2:
            return float('-inf')

        # 2. Evaluate zero on all training data (use simplified for speed)
        ...
        # 3. Compute fitness: reward node_count and variable diversity
        # If node_count is too small, penalize heavily to avoid trivial identities
        nodes = node.node_count()
        if nodes <= 4:
            return -1000 + nodes
        else:
            return nodes + (len(variables) * 2)
    ...
    def evolve(self, data_list):
        ...
        for gen in range(self.generations):
            ...
            if fitnesses[max_idx] > self.best_fitness:
                ...
                self.best_individual = self.population[max_idx].clone()
            ...
            if len(self.population) >= 2:
                second_idx = max([i for i in range(len(fitnesses)) if i != max_idx], key=lambda i: fitnesses[i])
...
def main():
    print("="*72)
    print("AEGIS‑Theorem v19 – reward any zero identity, penalize trivial")
    print("="*72)
    bounds = [3, 4, 5]
    ...
    gp = GeneticProgram(pop_size=600, generations=400, max_depth=8)

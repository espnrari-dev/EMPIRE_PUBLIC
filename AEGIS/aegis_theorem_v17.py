...
"""
AEGIS‑Theorem v17 – reward complexity, push toward non‑trivial identities.
"""
...
class GeneticProgram:
    def __init__(self, pop_size=500, generations=300, max_depth=8):
        ...
        self.best_fitness = float('-inf')  # maximize
    ...
    def fitness(self, node, data_list):
        ...
        # 3. Reject modulo by zero
        def has_mod_zero(n):
            if n.is_terminal():
                return False
            if n.value == '%' and n.right is not None and n.right.is_terminal() and n.right.value == 0:
                return True
            return (has_mod_zero(n.left) if n.left else False) or (has_mod_zero(n.right) if n.right else False)
        if has_mod_zero(simplified):
            return float('-inf')

        # 4. Must evaluate to zero on ALL training bounds
        for data in data_list:
            for x, y, z in data:
                if simplified.evaluate_int(x, y, z) != 0:
                    return float('-inf')

        # 5. Novelty filter: reject trivial patterns
        rep = repr(simplified)
        ...
        # 6. Fitness = node_count (we want larger node_count)
        # But we also add a small bonus for using more variables
...
def main():
    print("="*72)
    print("AEGIS‑Theorem v17 – reward complexity, max_depth=8")
    print("="*72)
    bounds = [3, 4, 5]
